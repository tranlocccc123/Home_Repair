	...
	public List<Images> findAll() {
	 ...
	 public void updateMainStatus(int serviceId, int mainStatus) {
	  ...
	  public String save(Images newItem) {
	        try {
	            String str_query = String.format("INSERT INTO tbl_images (ImageName, MainStatus, ServiceID) VALUES (?, ?, ?)");
	            int rowaccept = db.update(str_query, new Object[]{
	                    newItem.getImageName(),
	                    newItem.getMainStatus(),
	                    newItem.getServiceID()
	            });
	            if (rowaccept == 1) {
	                return "success";
	            }
